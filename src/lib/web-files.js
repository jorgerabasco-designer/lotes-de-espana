...
// ---------- LOTE METADATA (títulos y descripciones scrapeados de la web) ----------

// Todas las filas de lote_metadata. Se usa desde el PDF para pillar el título.
export async function listLoteMetadata() {
  if (!SUPABASE_READY) return [];
  const { data, error } = await supabase.from('lote_metadata').select('*').order('numero');
  if (error) throw new Error(error.message);
  return data || [];
}

// Devuelve la fila para un número de lote concreto (o null).
export async function getLoteMetadata(numero) {
  if (!SUPABASE_READY || !numero) return null;
...
// Estado actual de la última sincronización (settings.lote_metadata_sync).
//   { status: 'idle'|'running'|'done'|'failed', started_at, finished_at,
//     total, done, upserted, errors, message }
export async function getSyncStatus() {
  if (!SUPABASE_READY) return null;
  const { data } = await supabase
    .from('settings')
    .select('value')
    .eq('key', 'lote_metadata_sync')
    .maybeSingle();
  return data?.value || null;
}

// Dispara la función background que hace scraping de lotesdeespana.es.
// Devuelve 202 al instante; el progreso se ve en settings.lote_metadata_sync.
export async function triggerScrape() {
  const res = await fetch('/api/sync-lote-metadata', { method: 'POST' });
  if (res.status !== 202 && !res.ok) {
    const t = await res.text().catch(() => '');
    throw new Error(`No se pudo iniciar la sincronización (${res.status}): ${t}`);
  }
  return { ok: true };
}
